...
#%% find following month to current
...
#%% storage function
def outlier_dict_storage(ds, ds_variables, dt64_year, i, outliers, 
    ...
    outlier_upper = outliers.sel(time = slice(dt64_year[i], dt64_year[i+1]))
    ...
    if outlier_lower_type is not None:
        outlier_lower = outliers.sel(time = slice(dt64_year[i], dt64_year[i+1]))
