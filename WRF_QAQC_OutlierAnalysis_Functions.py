...
def rename_z_stats(zscore_ds, z_outlier_upper, z_outlier_lower, z_outlier_upper_inv, z_outlier_lower_inv, z_threshold, ds_variables):
    """
        Function for renaming the xarray variables for mean, med, min, max and std dev
    ...
    for i in range(length):
        ...
        z_threshold = z_threshold.rename({ds_variables[i]: f"{ds_variables[i]}_z_threshold"})
...
def z_outlier_storage(ds, ds_variables, zscore_ds, z_outlier_upper, z_outlier_lower, z_threshold):
    z_outlier_upper_dict = {i: np.where(z_outlier_upper[i].isnull()) for i in ds_variables}
    z_outlier_lower_dict = {i: np.where(z_outlier_lower[i].isnull()) for i in ds_variables}
